# FLNext-SDD 门控规则说明 v4.0

## 门控机制概述

FLNext-SDD 采用多层级门控机制，结合严格确认词、双编译门禁、集成门控与对抗性评审。
...
| 测试用例 | Phase B 编译通过 |
| 功能测试 | 测试用例 CONFIRMED |
| **集成门控 (7b)** | **功能测试 PASS + Phase A/B 均为 PASSED** |
| 提测 | **集成门控 CONFIRMED** + feature rebase develop 成功 |
| 验收 | 提测 CONFIRMED |
| 发布 | 验收 PASS |
...
| 功能测试 | PASS/REWORK/HOLD | PASS 才能进入集成门控 |
| 集成门控 | PASS/REWORK | PASS 才能提测 |
...
| Integration | 7b 集成门控 | Phase A/B 同时 PASSED + 全栈联调 | 路由回对应开发/测试阶段 |

**关键规则**: 编译失败后 AI 不得自动修复循环，必须展示错误、分析原因、等待人工确认。
...
```
PENDING → IN_PROGRESS → AWAITING_CONFIRMATION → CONFIRMED → COMPLETE
                                                        ↘
...
| CONFIRMED → COMPLETE | 项目最终完成（仅阶段 10） |
| PENDING → SKIPPED | 项目配置自动跳过（如 backend_only 跳过阶段 5） |

**注意**: CONFIRMED 状态不可回退（除非整体回滚）；SKIPPED 状态等同于已通过，不参与门控检查。
